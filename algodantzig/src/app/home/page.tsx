...
export default function Home() {
  ...
  // Fonction pour calculer le chemin le plus court avec l'algorithme de Dijkstra (Minimisation)
  ...
  // Fonction pour calculer le chemin le plus long (Maximisation)
  const calculateLongestPath = (nodes: Node[], edges: Edge[]) => {
    const distances: { [key: string]: number } = {};
    const predecessors: { [key: string]: string | null } = {};
    ...
    for (let i = 0; i < nodes.length - 1; i++) {
      edges.forEach(edge => {
        ...
        const cost = parseInt(edge.label?.toString() || "0");
        const newDist = distances[source] + cost;
    ...
    // Vérifier les cycles positifs (infinis)
    for (const edge of edges) {
      const source = edge.source;
      const target = edge.target;
      const cost = parseInt(edge.label?.toString() || "0");
      if (distances[source] + cost > distances[target]) {
        alert("Le graphe contient un cycle positif, le coût maximum est infini !");
  ...
  const handleCalculateShortestPath = () => {
    ...
    const updatedNodes = nodes.map((node) => ({
      ...node,
      data: { 
        label: `${node.data.label.split(' ')[0]} ${node.data.label.split(' ')[1]} (λ${node.id}=${distances[node.id] === Infinity ? '∞' : distances[node.id]})` 
      },
    }));
    ...
    alert(`Chemin optimal (Minimisation) : ${path.map(id => `X ${id}`).join(" -> ")}\nCoût total : ${totalCost}`);
  };

  // Fonction pour exécuter l'algorithme de maximisation et tracer le chemin optimal
  const handleCalculateLongestPath = () => {
    const { distances, predecessors } = calculateLongestPath(nodes, edges);

    if (!distances || !predecessors) {
      return; // Arrêt si un cycle positif est détecté
    }

    // Mettre à jour les labels des nœuds avec les distances μ
    const updatedNodes = nodes.map((node) => ({
      ...node,
      data: { 
        label: `${node.data.label.split(' ')[0]} ${node.data.label.split(' ')[1]} (μ${node.id}=${distances[node.id] === -Infinity ? '−∞' : distances[node.id]})` 
      },
    }));
    setNodes(updatedNodes);
    ...
    // Mettre à jour les arêtes pour colorer le chemin optimal en vert
    const updatedEdges = edges.map((edge) => ({
      ...edge,
      style: pathEdges.includes(edge.id) ? { stroke: "green", strokeWidth: 2 } : { stroke: "#000" },
    }));
    setEdges(updatedEdges);
    setOptimalPathEdges(pathEdges);
    ...
    // Afficher le chemin et le coût total
    const totalCost = distances["16"] === -Infinity ? "−∞" : distances["16"];
    alert(`Chemin optimal (Maximisation) : ${path.map(id => `X ${id}`).join(" -> ")}\nCoût total : ${totalCost}`);
  };
      ...
      <div className="grid grid-cols-[5fr_3fr_2fr] gap-4">
        ...
        <div className="border rounded-lg shadow-lg p-4 flex flex-col bg-white">
          ...
          <div className="flex gap-2 space-x-2">
            <Button
              onClick={handleCalculateLongestPath}
